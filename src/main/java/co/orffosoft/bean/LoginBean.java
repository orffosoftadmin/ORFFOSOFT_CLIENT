...
public class LoginBean implements Serializable {
	...
	private static final String APPNAME_PORTAL = "ORFFOSOFT";
	...
	@Getter
	@Setter
	List<LoginResponseDTO> listLoginResponseDTO;
	...
	public String submit() {
		...
		try {
			...
			listLoginResponseDTO=loginResponseDTO.getListLoginResponseDTO();
			expireditemlist=loginResponseDTO.getExpireditemlist();
			
			if (loginResponseDTO.getEntityMaster() != null) {
				initialStockUpoloaded = loginResponseDTO.getEntityMaster().isInitial_Stock_Updated();
			}
			password = "";
	...
	public String getDashBoardDetail() {
		...
		String restURL = serverURL + "/login/getDashBoardDetails";

		log.info(" LoginBean getDashBoardDetails >> restURL: " + restURL);
		try {
			
			RestTemplate restTemplate = new RestTemplate();
			ResponseEntity<BaseDTO> baseDTO = restTemplate.exchange(restURL, HttpMethod.GET, requestEntity, BaseDTO.class);
			if (baseDTO.getBody() != null) {
				ObjectMapper mapper = new ObjectMapper();
				String jsonResponse = mapper.writeValueAsString(baseDTO.getBody().getResponseContent());
				LoginResponseDTO loginDto = mapper.readValue(jsonResponse,LoginResponseDTO.class);
				if (loginDto != null) {
					loginResponseDTO.setTotalSaleAmount(loginDto.getTotalSaleAmount());
					loginResponseDTO.setTotalReturnAmt(loginDto.getTotalReturnAmt());
					loginResponseDTO.setTotalbill(loginDto.getTotalbill());
				}
				log.info("Total Sales Amount Is == ", loginResponseDTO.getTotalSaleAmount());
			}
			if (baseDTO.getBody() != null) {
				ObjectMapper mapper = new ObjectMapper();
				String jsonResponse = mapper.writeValueAsString(baseDTO.getBody().getResponseContents());
				listLoginResponseDTO = mapper.readValue(jsonResponse, new TypeReference<List<LoginResponseDTO>>() {
				});
			
				log.info("Total Sales Amount Is == ", loginResponseDTO.getTotalSaleAmount());
			}
		} catch (Exception e) {
			log.error("Exception Occured on LoginBean >> getDashBoardDetails >> " + e);
		}
		return defaultLandingPage;
	}
	...
	public Double calculateTodaySale() {
		Double totalSaleAmount = null;
		try {

			String url = SERVER_URL + "/login/totalSaleAmount/";

			HttpEntity<?> httpRequestEntity = new HttpEntity<>(headers);

			ResponseEntity<Double> responseEntity = restTemplate.exchange(url, HttpMethod.GET, httpRequestEntity,
					Double.class);

			totalSaleAmount = responseEntity.getBody();
			loginResponseDTO.setTotalSaleAmount(totalSaleAmount);

		} catch (Exception e) {
			log.error(" Exception calculateTodaySale === >> ");
		}
